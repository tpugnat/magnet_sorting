...
list_dtf_q2 = [[-2.986934295978083, 3, 'gaussian'],
 [19.031038514386562, 3, 'gaussian'],
 [-15.958763238518092, 10, 'uniform'],
 [-7.936711129315063, 10, 'uniform'],
 [-3.4989801752888887, 10, 'uniform'],
 [11.350350324724477, 3, 'gaussian'],
 [0, 20, 'gaussian'],
 [0, 20, 'gaussian']]

if FLAG_WITH_calibration:
    NAME_SAVEFILE_FORMAT = f"{TYPE_SCORE}_dist_r{AMP_MEAS_ERROR}u_{TYPE_RAND}_c{AMP_CALI_ERROR}u_p{AMP_PRES_ERROR}u_nbsimu{MAX_SIMS}_wcor_MEASNOCOLDUNKNOWVALUESQ2_Phase{STAGE}"
else:
    NAME_SAVEFILE_FORMAT = f"{TYPE_SCORE}_dist_r{AMP_MEAS_ERROR}u_{TYPE_RAND}_c{AMP_CALI_ERROR}u_p{AMP_PRES_ERROR}u_nbsimu{MAX_SIMS}_MEASNOCOLDUNKNOWVALUESQ2_Phase{STAGE}"
...
#         optic_without_error = None
    ...
    q2_errors = Q2Pairs(meas_error = AMP_MEAS_ERROR, cali_error = cerror, 
                        pres_error = 3*AMP_PRES_ERROR, 
                        stage=STAGE, optic = optic_without_error)
#     q2_errors.set_magnets_real_strength(list_dtf_q2)
#     q2_errors.set_magnets_meas_strength(list_dtf_q2)
#     q2_errors.set_meas_strength_no_cold_incertitudes_for_sorting(list_dtf_q2)
    q2_errors.set_meas_strength_no_cold_nor_unknown_incertitudes_for_sorting(list_dtf_q2)
    
    cerror = uniform(AMP_CALI_ERROR)
    cerror = AMP_CALI_ERROR*cerror/abs(cerror) if (cerror != 0) else AMP_CALI_ERROR
    q1_errors = Q1Pairs(meas_error = AMP_MEAS_ERROR, cali_error = cerror, 
                        pres_error = 3*AMP_PRES_ERROR, 
                        stage=STAGE, optic = optic_without_error)
#     q1_errors.set_magnets_real_strength(list_dtf_q1)
#     q1_errors.set_magnets_meas_strength(list_dtf_q1)
#     q1_errors.set_meas_strength_no_cold_incertitudes_for_sorting(list_dtf_q1)
#     q1_errors.set_meas_strength_no_cold_nor_unknown_incertitudes_for_sorting(list_dtf_q1)
...
#                 raise TypeError(f"TYPE_ERROR for the choise of sorting must be either REAL or MEAS")
    ...
    try:
        ...
        q1_errors.initial_permutation, q2_errors.initial_permutation = prepare_data_for_analysis(q1_errors, q2_errors)
