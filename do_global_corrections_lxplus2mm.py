...
if FLAG_WITH_calibration:
    NAME_SAVEFILE_FORMAT = f"{TYPE_SCORE}_dist_r{AMP_MEAS_ERROR}u_{TYPE_RAND}_c{AMP_CALI_ERROR}u_MQXFA{SIGN_MQXFA}-MQXFB{SIGN_MQXFB}_p{AMP_PRES_ERROR}u_nbsimu{MAX_SIMS}_wcor_Phase{STAGE}"
else:
    NAME_SAVEFILE_FORMAT = f"{TYPE_SCORE}_dist_r{AMP_MEAS_ERROR}u_{TYPE_RAND}_c{AMP_CALI_ERROR}u_MQXFA{SIGN_MQXFA}-MQXFB{SIGN_MQXFB}_p{AMP_PRES_ERROR}u_nbsimu{MAX_SIMS}_Phase{STAGE}"
...
def write_permutation_table(q1_errors, q2_errors, summ):
    if not os.path.exists(f"permutation_s{summ.total():d}_{NAME_SAVEFILE_FORMAT}.tfs"):
...
#         optic_without_error = None
    ...
    if SIGN_MQXFB == 'p':
        ...
        raise ValueError(f"{SIGN_MQXFB=} instead of 'p', 'm' or ''!")
    ...
    if SIGN_MQXFA == 'p':
        ...
        raise ValueError(f"{SIGN_MQXFA=} instead of 'p', 'm' or ''!")
